...
public class Bezier extends Primitive{
    ...
    Point bezierCurve(double t){
        return GeometryUtils.add(
                    GeometryUtils.add( GeometryUtils.multiply(points[0],(Math.pow(1-t,3))),
                        GeometryUtils.multiply(points[1],3*t*Math.pow(1-t,2))
                    ),
                     GeometryUtils.add(
                        GeometryUtils.multiply(points[2],3*(1-t)*Math.pow(t,2)), GeometryUtils.multiply(points[3],Math.pow(t,3))
                     )
        );
    }
    ...
    public void draw() {
        ...
        for(double t=0;t<=1;t+=precision){
            ...
            if(isInsideScene(pointOnCurve.getX(), pointOnCurve.getY())){
                sceneField[pointOnCurve.getX()][pointOnCurve.getY()]=charToDraw;
            }
        }
